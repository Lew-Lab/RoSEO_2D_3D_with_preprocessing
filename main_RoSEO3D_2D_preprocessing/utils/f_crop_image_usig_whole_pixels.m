function f_crop_image_usig_whole_pixels(dataN,fileFolder,inter_results_save_folder,saveData_folder_Name,ROI_centerY,W,Nimg,FoV,N_FoV,FoV_each,tform_FoV_each,crop_data_pre_name)
W = W/2;

for mm = 1:length(dataN)
%fileFolder = ['E:\Experimental_data\20220814 A1-LCD\'];
%SMLMName = strcat(fileFolder, crop_data_pre_name,'\', crop_data_pre_name,'_MMStack_Default', '_',num2str(dataN(mm)),'.ome.tif');
...
tformLoad = load(strcat(saveData_folder_Name, '\tformx2y_y_center_', num2str(ROI_centerY_cur_old(1)),'_', num2str(ROI_centerY_cur_old(2)), '_FoV_', num2str(tform_FoV_each),'.mat'));
1
...
SMLM_img = [uint16(SMLM_img_ROIx),uint16(SMLM_img_ROIy)];
    ...
    if i==1
     imwrite(SMLM_img,strcat(saveData_folder_Name,SMLM_save_Nmae))
    else
    imwrite(SMLM_img,strcat(saveData_folder_Name,SMLM_save_Nmae),'WriteMode','append')
    end
end
end
