function f_tform_use_sample(fileFolder,W,pixel_sz,photonThred,frame_thred,ratio_y2x,center_global,ROI_each,ROI_whole,dataN,N_FoV,filter_close_emitter,inter_results_save_folder,xy_ch_list_together,SMLM_image_save_folder,beads_folder,beads_data,tform_data_pre_name)
%% requirement:
...
%----------if use beads-------------
%[tform_y2x,tformx2y] = f_tform_use_beads(beads_folder,W,pixel_sz,photonThred,frame_thred,ratio_y2x,center_global,ROI_each,ROI_whole,beads_data,N_FoV,filter_close_emitter,inter_results_save_folder,xy_ch_list_together,SMLM_image_save_folder);
%----------if use current sample-------------
[tform_y2x,tformx2y] = f_tform_use_beads(fileFolder,W,pixel_sz,photonThred,frame_thred,ratio_y2x,center_global,ROI_each,ROI_whole,dataN,N_FoV,filter_close_emitter,inter_results_save_folder,xy_ch_list_together,SMLM_image_save_folder);
%%
W = W/2;
%% read the file name
...
fileName = fullfile(fileFolder , ['data' num2str(dataN(jj)) '_xch' char(tform_data_pre_name) '.csv']);
    ...
    frameX = dataX(end,1);
fileName = fullfile(fileFolder ,  ['data' num2str(dataN(jj)) '_ych' char(tform_data_pre_name) '.csv']);
    ...
    fileName = fullfile(fileFolder , ['data' num2str(dataN(jj)) '_xych' char(tform_data_pre_name) '.csv']);
...
exportgraphics(gca,strcat(inter_results_save_folder,'\image used for registration.jpg'),'Resolution',600);
...
for jj = 1:length(center_X)
    ...
    centerY_cur = center_global+[center_x(jj),center_y(jj)];
    centerY_cur(1) = W-centerY_cur(1);
    
    centerX_cur = transformPointsInverse(tformx2y,centerY_cur);
...
%shrink ROI
dataX(dataX(:,2)<(centerX_cur(1)-round(ROI_each/2)) | dataX(:,2)>(centerX_cur(1)+round(ROI_each/2)),:)=[];
dataX(dataX(:,3)<(centerX_cur(2)-round(ROI_each/2)) | dataX(:,3)>(centerX_cur(2)+round(ROI_each/2)),:)=[];

dataY(dataY(:,2)<(centerY_cur(1)-round(ROI_each/2)) | dataY(:,2)>(centerY_cur(1)+round(ROI_each/2)),:)=[];
dataY(dataY(:,3)<(centerY_cur(2)-round(ROI_each/2)) | dataY(:,3)>(centerY_cur(2)+round(ROI_each/2)),:)=[];
...
exportgraphics(gca,strcat(inter_results_save_folder,'\subFoV for tform',num2str(jj),'.jpg'),'Resolution',600);


%% filter out closely closed emitters
if filter_close_emitter == 1
...
Fig1 = figure('Visible','off','Units','inches','InnerPosition',[1,1,5,5]);
histogram(distance);  xlabel('distance'); title('distance when applying final poly-tform');
exportgraphics(Fig1,strcat(inter_results_save_folder,'\tform_results histogram',num2str(jj),'.jpg'),'Resolution',100);
...
exportgraphics(Fig1,strcat(inter_results_save_folder,'\tform_results poly',num2str(jj),'.jpg'),'Resolution',600);
...
exportgraphics(Fig1,strcat(inter_results_save_folder,'\tform_results linear',num2str(jj),'.jpg'),'Resolution',600);


save(strcat(SMLM_image_save_folder,'\tformx2y_y_center_',num2str(W-centerY_cur(1)),'_',num2str(centerY_cur(2)),'_FoV_',num2str(ROI_each),'.mat'),'tformx2y');
end
%% function
