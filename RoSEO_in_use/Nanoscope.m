classdef Nanoscope
    ...
    methods (Static)
        ...
        function phaseMask_out = mountPhaseMask(obj, varargin)
            ...
            maskSize = 596;%round((obj.pixelSize)^-1*(obj.emissWavelength * rho_max)/obj.numApt);
            if mod(maskSize, 2) ~= 0
                maskSize = maskSize + 1;
            end
    ...
    methods (Static)
        ...
        function [imgx, imgy, Ex, Ey] = simDipole_novotny(Nanoscope, Emitter, varargin)
            ...
            % use calibrated phase mask instead if there are field named
            % calibrated mask
            if isfield(Nanoscope.phaseMaskPara,'calibrated_mask')
                load(Nanoscope.phaseMaskPara.calibrated_mask);
                %place the pmask into the center of the BFP
                pmask_x = pmask_x*0;
                pmask_y = pmask_y*0;
                if size(pmask_x,1)>=size(pmaskX,1)
                    range = (0:size(pmaskX,1)-1)-floor(size(pmaskX,1)/2)+floor(size(pmask_x,1)/2)+1;
                    pmask_x(range,range)=pmaskX;
                    pmask_y(range,range)=pmaskY;
                else
                    range = (0:size(pmask_x,1)-1)-floor(size(pmask_x,1)/2)+floor(size(pmaskX,1)/2)+1;
                    pmask_x=pmaskX(range,range);
                    pmask_y=pmaskY(range,range);
                end
                pmask_x = exp(1j*pmask_x);
                pmask_y = exp(1j*pmask_y);
            end
    ...
    methods
        ...
        function h = visualizeBases(obj, varargin)
