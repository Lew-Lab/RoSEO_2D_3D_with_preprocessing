...
z_offset =0;
x_cord = x; y_cord = y;z_cord = z-z_offset;  %-90
...
indxN = abs(x_cord)<1200 & abs(y_cord)<1200 & z_cord<900 & z_cord>200 &SM_est_final(:,5)>2000;
[R,C]=sphereFit([x_cord(indxN), y_cord(indxN),z_cord(indxN)])
...
%% lateral location precision quantification in terms of z
...
half_max= max_Data/2;
leftBin = find(pdf_data > half_max, 1, 'first');
rightBin = find(pdf_data > half_max, 1, 'last');
...
%data_FWHM(ii) = center_data(rightBin)-center_data(leftBin);
...
%% lateral location precision quantification in terms of phi
% reference: perfect pixOL precision
load('pixOL_com_loc_precision_NFP-700.mat');
sigma_r = sqrt(sigma_x_opt.^2+sigma_y_opt.^2);
...
figure();
plot((phiSlice(1:end-1)+phiSlice(2:end))/2,data_FWHM);
xlabel('\phi (\circ)'); ylabel('count');
xlim([-180,180]);
%% plot the FWHM
