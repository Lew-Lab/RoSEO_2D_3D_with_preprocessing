function loc_data = get_loc_data2(gammaf, loc_rec,scaling_factor)
loc_data = [];
...
for ii = 1:size(gammaf,2)
     ...
     pm = loc_rec(:,ii);
     %-----------------------------------------------------
    imx_xx = (z_xx(ii,2) / (eps + z_xx(ii,1))) * scaling_factor(1); %x position
    imy_xx = (z_xx(ii,3) / (eps + z_xx(ii,1))) * scaling_factor(2); %y position
    imz_xx = (z_xx(ii,4) / (eps + z_xx(ii,1))) * scaling_factor(3); %z position
    ...
    imx_yy = (z_yy(ii,2) / (eps + z_yy(ii,1))) * scaling_factor(1); %x position
    imy_yy = (z_yy(ii,3) / (eps + z_yy(ii,1))) * scaling_factor(2); %y position
    imz_yy = (z_yy(ii,4) / (eps + z_yy(ii,1))) * scaling_factor(3); %z position
    br_m_yy = z_yy(ii,1); %brightness scales

    % ZZ basis
    %-----------------------------------------------------

    imx_zz = (z_zz(ii,2) / (eps + z_zz(ii,1))) * scaling_factor(1); %x position
    imy_zz = (z_zz(ii,3) / (eps + z_zz(ii,1))) * scaling_factor(2); %y position
    imz_zz = (z_zz(ii,4) / (eps + z_zz(ii,1))) * scaling_factor(3); %z position
