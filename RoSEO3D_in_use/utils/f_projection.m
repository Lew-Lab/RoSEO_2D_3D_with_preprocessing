function gammaNew =f_projection(gamma,rx,rz,scaling_factor)

ratioN = scaling_factor(3)/scaling_factor(1);
...
xxjoint = sqrt(xxdx.^2+xxdy.^2+xxdz.^2*(ratioN).^2);
yyjoint = sqrt(yydx.^2+yydy.^2+yydz.^2*(ratioN).^2);
zzjoint = sqrt(zzdx.^2+zzdy.^2+zzdz.^2*(ratioN).^2);
...
%**************
%XX
cxx = (xx + rx * xxjoint) / (1 + rx^2);
cxx1 = xxjoint <= xx * rx;
cxx2 = xxjoint <= -xx / rx;
cxx3 = xxjoint > abs(xx*rx);
cxx_t1 = (1 - cxx2) .* cxx1;
cxx_t2 = (1 - cxx2) .* cxx3 .* cxx;

projected_gammaxx_t = cxx_t1 .* xx + cxx_t2;
projected_gammaxx_tx = cxx_t1 .* xxdx + cxx_t2 .* rx .* (xxdx) ./ (eps + xxjoint);
projected_gammaxx_ty = cxx_t1 .* xxdy + cxx_t2 .* rx .* xxdy ./ (eps + xxjoint);
projected_gammaxx_tz = cxx_t1 .* xxdz + cxx_t2 .* rx .* xxdz ./ (eps + xxjoint);
projected_gammaxx_t = projected_gammaxx_t .* (projected_gammaxx_t > 0);


%YY
cyy = (yy + rx * yyjoint) / (1 + rx^2);
cyy1 = yyjoint <= yy * rx;
cyy2 = yyjoint <= -yy / rx;
cyy3 = yyjoint > abs(yy*rx);
cyy_t1 = (1 - cyy2) .* cyy1;
cyy_t2 = (1 - cyy2) .* cyy3 .* cyy;

projected_gammayy_t = cyy_t1 .* yy + cyy_t2;
projected_gammayy_tx = cyy_t1 .* yydx + cyy_t2 .* rx .* (yydx) ./ (eps + yyjoint);
projected_gammayy_ty = cyy_t1 .* yydy + cyy_t2 .* rx .* yydy ./ (eps + yyjoint);
projected_gammayy_tz = cyy_t1 .* yydz + cyy_t2 .* rx .* yydz ./ (eps + yyjoint);
projected_gammayy_t = projected_gammayy_t .* (projected_gammayy_t > 0);



%ZZ
czz = (zz + rz * zzjoint) / (1 + rz^2);
czz1 = zzjoint <= zz * rz;
czz2 = zzjoint <= -zz / rz;
czz3 = zzjoint > abs(zz*rz);
czz_t1 = (1 - czz2) .* czz1;
czz_t2 = (1 - czz2) .* czz3 .* czz;

projected_gammazz_t = czz_t1 .* zz + czz_t2;
projected_gammazz_tx = czz_t1 .* zzdx + czz_t2 .* rz .* (zzdx) ./ (eps + zzjoint);
projected_gammazz_ty = czz_t1 .* zzdy + czz_t2 .* rz .* zzdy ./ (eps + zzjoint);
projected_gammazz_tz = czz_t1 .* zzdz + czz_t2 .* rz .* zzdz ./ (eps + zzjoint);
