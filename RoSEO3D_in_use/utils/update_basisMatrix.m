function [G_re,loc_rec,gamma_new] = update_basisMatrix(N,gamma,loc_rec,imgPara,imgParaB)

SM_est= get_loc_data2(gamma, loc_rec,imgPara.scaling_factor);
...
for ii = 1:N 
    ...
    gamma_new(:,ii) = gamma_update_2_loc_rec([ind_x_add,ind_y_add,ind_z_add].*[pix_sizex,pix_sizex,pix_sizez],gamma(:,ii),imgPara.scaling_factor);
    
    indx_trans = round((B_size+1)/2)+1+round([-(img_sizex-1)/2:(img_sizex-1)/2])-ind_x;
    indy_trans = round((B_size+1)/2)+1+round([-(img_sizey-1)/2:(img_sizey-1)/2])-ind_y;
